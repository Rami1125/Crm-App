...
async function postData(action, payload) {
    try {
        const response = await fetch(API_URL, {
            method: 'POST',
            // mode: 'no-cors', // Use this only for debugging if CORS issues persist
...
document.addEventListener('DOMContentLoaded', () => {
    // ⭐ FIX: Add a small delay to ensure everything is ready before the first fetch
    setTimeout(() => {
        loadActiveClients();
        loadRecentRequests();
        setInterval(loadRecentRequests, 30000); // Poll for new requests less aggressively
    }, 500);
});
